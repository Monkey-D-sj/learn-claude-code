"""recall —— 把被压掉的那段原文按号取回来。

号是 sessions.db 里那一行的行号:agent.py 落库时拿到它、拼进结果正文的尾巴。
所以**"有号"就等于"这段查得回来"** —— 写不进去的结果没有行号,也就没有号,
模型看不见它,自然也不会去点它。
...
取回器怎么到 handler 手上,见下面 _RECALL —— contextvar,不是参数。理由跟
context.bind_messages 一模一样:handler 只拿得到 **block.input。
"""
...
# 会被它们串成一份 —— 而串了不报错,只是 A 会话的号在 B 会话里查到了别人的原文。
