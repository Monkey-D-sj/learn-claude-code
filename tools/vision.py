...
**为什么是"就地消化"而不是把图放进对话:** 后者要动 context.py 的压缩器。
当初拦下它的是那把尺子 —— 拿仓库根目录那张 153 KB 的 jpg 实测过:

	(旧尺子,量字符)estimate_chars = 204574,而 CONTEXT_CHAR_LIMIT = 50000

于是一张图就让压缩器以为超了 4 倍,第一轮就把 tool_result 换成一句指针 ——
**图没了**,而且"落盘"的那份是 str(list) 出来的 Python repr(单引号),
永远发不回 API。token 那头反倒没事:那张图才 667 输入 token。

**这两个数现在都过期了**(2026-09-21 尺子换成了按类估 token:estimate_tokens
+ CONTEXT_TOKEN_BUDGET = 300,000)。同一张图按新尺子约 5.6 万 token,不再越线 ——
也就是说当初那个具体的失效场景没了。**但"就地消化"这个设计不改**,理由是另外
两条仍然成立:图根本不进对话(重复问才需要重调,这个代价写在下面),以及主对话
里从头到尾只有文字,压缩器那一整套碰不到它。要重新评估这个设计,先把上面两个
数重量一遍 —— 别拿旧数字当依据。

所以图在这儿就地消化:handler 自己调一次模型,只把回答交回去。
