...
#
# **现在不设人为上限了(1000 ≈ 没有上限)。** 原来写 50,那是"够跑完一件
# 普通活"的经验值;它挡住的不是失控,而是**长活**——一件要读十几个文件、改
# 七八处、跑几遍测试的事,50 轮撞线之后循环直接停,前面几十轮的工作一个字都
# 交付不出去。要看它真实能干多少,先把这道闸放开。
#
# 代价说清楚:这一版没有钱和时间的闸。真正兜住成本的是 ROUND_WARN(收尾提醒)
# 和 ASK_TIMEOUT,以及你自己看得见的侧栏"在跑"——**一个被忘掉的标签页会把
# 这一轮的钱烧完**,现在能烧到 1000 轮。要防它得另做:每轮累计到多少 token
# 就停,那是另一件事(usage 账本里有数,缺的是回调)。
MAX_ROUNDS = 1000

# 剩这么多轮的时候开始提醒模型收尾。
#
# 不能等到只剩 1 轮:只会"该收尾了"是没用的,模型手里可能正拆到一半 —— 得给
# 它几步,把手上这一步做完,再写出一段交代得清楚的答复。只有最后一次调用才
# 收到提醒的话,它要么中断在半截,要么再调一个工具(那一次之后循环直接停,
# 前面几十轮的工作一个字都交付不出去)—— 那不是提醒,是最后通牒。
#
# 跟着 MAX_ROUNDS 一起放,理由相同:子 agent 也用这一份。
ROUND_WARN = 3
